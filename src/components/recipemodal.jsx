...
export default function RecipeModal({showFullRecipe, setShowFullRecipe}) {
    ...
    const ingredientsList = showFullRecipe.extendedIngredients.map(ingredient => {
    ...
    const methodData = showFullRecipe.analyzedInstructions.length > 0 ? showFullRecipe.analyzedInstructions[0].steps : false;
    ...
    <div className='recipe-modal-background'>   
        <div className="recipe-modal">
            ...
            <button className='btn' onClick={() => setShowFullRecipe('')}>Back to recipes</button>
        </div>
    </div> 
    )
}