...
export default function ShoppingList({recipeData, setIngredientDisplayStatus, ingredientDisplayStatus}) {
        ...
        data.forEach(recipe => {
            recipe.extendedIngredients.forEach(ingredient => {
                ...
                if (aisle in shoppingList) {
                    ...
                    shoppingList[aisle][ingredientName] = {'id': ingredientId, 'name': ingredientName, 'amount': [ingredientQuantity]};
                    }
                } else {
                    shoppingList[aisle] = {[ingredientName]:{'id': ingredientId, 'name': ingredientName, 'amount': [ingredientQuantity]}};
                }
    ...
    function sortIngredientsByRecipe(data){
        ...
        data.forEach(recipe => {
            ...
            recipe.extendedIngredients.forEach(ingredient => {
                ...
                const ingredientAmount = Math.round(ingredient.amount * 100)/100;
                const ingredientQuantity = `${ingredientAmount} ${ingredient.unit}`.trim(); 
                let newIngredient = {'id': ingredientId, 'name': ingredientName, 'amount': [ingredientQuantity]}
    ...
    const IngredientsByRecipeData = useMemo(() => sortIngredientsByRecipe(recipeData),[recipeData]) 
    const ingredientsByCategoryData = useMemo(() => sortIngredientsByCategory(recipeData),[recipeData]);
    ...
    const categoryOrderObject = useMemo(() => {
        ...
        const categoryOrder = ['Produce','Seafood','Meat','Refrigerated','Milk, Eggs, Other Dairy','Cheese','Beverages','Bakery/Bread','Cereal','Nut butters, Jams, and Honey','Baking','Nuts','Spices and Seasonings','Oil, Vinegar, Salad Dressing','Pasta and Rice','Condiments','Canned and Jarred','Ethnic Foods','Health Foods','Gluten Free','Gourmet','Alcoholic Beverages','Other'];
    ...
    .map(aisle => {
        const filteredIngredients = Object.keys(ingredientsByCategoryData[aisle]).filter(ingredient => {
            ...
            <ul id={aisle}>
             {filteredIngredients.map(ingredient => {
                return (
                    ...
                    name = {ingredient}
                    amount = {` (${ingredientsByCategoryData[aisle][ingredient].amount.join(' + ')})`}
    ...
    const IngredientsByRecipeElements = IngredientsByRecipeData
    .map(recipe => {
    ...
    return (
        <div className="shopping-list">
            ...
            {!showDeleted && <div className='sort-options'>
                <span>Sort by:</span>
                <button 
            ...
            <div className='btn-container'>
                <button className='btn' onClick={toggleShowDeleted}>{showDeleted ? 'Go back' : 'Deleted items'}</button>
            </div>
